...
def calculate(element: List[int | str], ap: int, parent: Tk):
    """
    ...
    # ---------------------------------------------------------------------------------------------------------------
        ...
        fetchSections(dir_path, element[0])
    # ---------------------------------------------------------------------------------------------------------------
    elif ap == 4:
...
def params(element: List[int | str]):
    """
...
class App(Tk):
    ...
    def __init__(self):
        ...
        def quit_window(element: List[int | str]):
            """
        ...
        self.topLabel = Label(self, text="", font=20) # type: ignore
        self.topLabel.grid(row=2, column=3, columnspan=10)

        self.Label1 = Label(self, text="Click the element for which you would like to obtain the atomic parameters.", font=22) # type: ignore
        self.Label1.grid(row=0, column=0, columnspan=18)

        self.Label2 = Label(self, text="", font=20) # type: ignore
        self.Label2.grid(row=8, column=0, columnspan=18)

        self.Label3 = Label(self, text="* Lanthanoids", font=20) # type: ignore
        self.Label3.grid(row=9, column=1, columnspan=2)

        self.Label4 = Label(self, text="** Actinoids", font=20) # type: ignore
        ...
        for i, element in enumerate(per_table):
            Button(self, text=element[3], width=5, height=2, bg=element[5], command=lambda i=i: quit_window([(i + 1), per_table[i][2]])).grid(row=element[6], column=element[7]) # type: ignore
