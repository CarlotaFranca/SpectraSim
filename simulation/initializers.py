...
def initialize_XYW(type_simu: str, ploted_cs: List[str] = []):
    ...
    x: List[List[float]] = [[] for _ in range(len(trans_dict) * cs_mult)]
    y: List[List[float]] = [[] for _ in range(len(trans_dict) * cs_mult)]
    w: List[List[float]] = [[] for _ in range(len(trans_dict) * cs_mult)]
